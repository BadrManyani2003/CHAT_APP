        ...
        <div class="relative">
          ...
          <label 
            ...
            <svg class="w-6 h-6 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 12h6M6 6h6m6 0h6m-6 6h6m-6 6h6"></path>
            </svg>
          </label>
        </div>
...
const onProfileImageChange = async (event) => {
  ...
  if (file) {
    ...
    try {
      await userStore.uploadProfileImage(user.value.id, formData);
      user.value.imgprofile = URL.createObjectURL(file); // Preview image
    } catch (error) {
