...
const openEditModal = () => {
  ...
  imageFile.value = null; // Reset the image file
...
const updateProfile = async () => {
  try {
    ...
    if (imageFile.value) {
      ...
      if (!validImageTypes.includes(imageFile.value.type)) {
        throw new Error('Le fichier sélectionné n\'est pas une image valide.');
      }

      const formDataToSend = new FormData();
      formDataToSend.append('imgprofile', imageFile.value);
      ...
      await userStore.updateUser(user.value.id, formDataToSend); // Utiliser FormData pour le téléchargement de fichiers
    } else {
      await userStore.updateUser(user.value.id, updatedData);
    }

    user.value = userStore.user; // Rafraîchir les données de l'utilisateur
    closeEditModal();
  } catch (error) {
