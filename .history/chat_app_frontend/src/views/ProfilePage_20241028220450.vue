        ...
        <div class="p-6">
          <form @submit.prevent="updateProfile" class="space-y-4">
            ...
            <div>
              ...
              />
              <img v-if="formData.imgprofile" :src="formData.imgprofile" class="mt-2 rounded-full w-24 h-24 object-cover" alt="Preview Image" />
            </div>
...
const openEditModal = () => {
  formData.value = { ...user.value }; // Initialize form data with user data
...
const handleImageUpload = (event) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      formData.value.imgprofile = e.target.result; // Preview the uploaded image
    };
...
const updateProfile = async () => {
  await userStore.updateUser(formData.value); // Assume updateUser handles the image upload
  user.value = { ...formData.value }; // Update user data in the component
  closeEditModal();
};

const confirmDelete = async () => {
  await userStore.deleteUser(); // Assume deleteUser handles the user deletion logic
  closeDeleteModal();
