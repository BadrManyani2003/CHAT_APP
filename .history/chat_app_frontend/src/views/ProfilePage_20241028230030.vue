...
const formData = ref({
  ...
  imgprofile: null, // Initialise imgprofile comme null pour l'image par défaut
});
...
const updateProfile = async () => {
  try {
    ...
    // Appel à la fonction de mise à jour de l'utilisateur
...
// Assurez-vous de mettre à jour `formData` avec les données utilisateur actuelles lors de l'ouverture du modal
const openEditModal = () => {
  formData.value = { 
    name: user.value.name, 
    email: user.value.email, 
    address: user.value.address, 
    gender: user.value.gender,
    imgprofile: null, // Réinitialise pour éviter de soumettre une ancienne image
  };
  isEditModalOpen.value = true;
};
