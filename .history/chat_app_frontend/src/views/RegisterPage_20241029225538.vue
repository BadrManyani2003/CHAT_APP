...
export default {
  data() {
    return {
      ...
      imgprofile: null, // Pour stocker l'image de profil
      errorMessage: ''
    };
  },
  methods: {
    onFileChange(event) {
      this.imgprofile = event.target.files[0]; // Récupérer le fichier sélectionné
      console.log('Image sélectionnée:', this.imgprofile); // Vérification
    },
    async register() {
      ...
      if (this.imgprofile) {
        formData.append('imgprofile', this.imgprofile); // Ajouter l'image à FormData
      } else {
