...
<script>
import api from '@/store/api'; // Assurez-vous que ce chemin est correct
...
export default {
  data() {
    return {
      ...
      imgprofile: null, // Pour stocker le fichier d'image
      errorMessage: ''
    };
  },
  methods: {
    onFileChange(event) {
      // Vérifiez que le fichier a été sélectionné
      if (event.target.files && event.target.files.length > 0) {
        this.imgprofile = event.target.files[0]; // Stocke le premier fichier
      } else {
        this.imgprofile = null; // Aucune image sélectionnée
      }
    },
    async register() {
      ...
      // Vérifiez que imgprofile n'est pas nul avant de l'ajouter
      if (this.imgprofile) {
