        ...
        <div class="p-6">
          <form @submit.prevent="updateProfile" class="space-y-4">
            ...
            <div>
              ...
              <input type="file" @change="onImageChange" accept="image/*" id="imgprofile" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg block w-full p-2.5" />
            </div>
...
const openEditModal = () => {
  ...
  selectedImage.value = null; // Reset selected image when opening modal
...
const onImageChange = (event) => {
  ...
  if (file) {
    ...
    reader.onload = (e) => {
      user.value.imgprofile = e.target.result; // Show preview
    };
    reader.readAsDataURL(file);
    selectedImage.value = file; // Store the selected image for upload
  }
};
...
const updateProfile = async () => {
  try {
    ...
    if (selectedImage.value) {
      ...
      await userStore.uploadProfileImage(formDataObj); // Assume a method exists for image upload
    }
    await userStore.updateUser(user.value.id, updatedData);
    user.value = userStore.user;
    closeEditModal();
  } catch (error) {
