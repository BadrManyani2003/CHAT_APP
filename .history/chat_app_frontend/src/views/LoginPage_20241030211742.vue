        ...
        <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
          ...
          <form class="space-y-4 md:space-y-6" @submit.prevent="handleLogin">
            ...
            <p class="text-sm font-light text-gray-500 dark:text-gray-400">
              ...
              <router-link to="/register" class="font-medium text-primary-600 hover:underline dark:text-primary-500">S'inscrire</router-link>
            </p>
          </form>
        </div>
      </div>
  ...
  setup() {
    ...
    const handleLogin = async () => {
      ...
      try {
        ...
        // Vérifiez si l'utilisateur est authentifié et redirigez
        if (userStore.isAuthenticated) {
          router.push('/'); // Redirection vers le tableau de bord après connexion
        } else {
          errorMessage.value = 'Échec de la connexion, veuillez vérifier vos informations.'; // Message d'erreur si l'authentification échoue
        }
      } catch (err) {
        console.error('Login failed:', err);
        errorMessage.value = 'Échec de la connexion, veuillez vérifier vos informations.'; // Message d'erreur par défaut
      }
    };
