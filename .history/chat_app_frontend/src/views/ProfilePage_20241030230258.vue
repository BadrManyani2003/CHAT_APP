<template>
  <div class="container mx-auto p-6">
    ...
    <BaseModal v-if="isEditModalOpen" @close="closeEditModal">
      <template #header>
    ...
    <BaseModal v-if="isDeleteModalOpen" @close="closeDeleteModal">
      <template #header>
...
export default {
  ...
  setup() {
    ...
    const editUserData = ref({ ...userStore.user });
    ...
    const updateProfile = async () => {
      try {
        await userStore.updateUser(user.value.id, editUserData.value);
        closeEditModal();
      } catch (error) {
